...
namespace UnityFracture
{
    public sealed class Model
    {
        ...
        public Model(Mesh mesh, Material[] materials, Transform transform)
        {
            ...
            m_Vertices = VertexUtills.GetVertices(mesh).Select(x => transform.TransformVertex(x)).ToList();
        ...
        internal Model(List<Polygon> polygons)
        {
            ...
            for (int i = 0; i < polygons.Count; i++)
            {
                ...
                if (!submeshes.TryGetValue(poly.mat, out indices))
                    submeshes.Add(poly.mat, indices = new List<int>());
        ...
        {
            var mesh = new Mesh();
            VertexUtills.SetMesh(mesh, model.m_Vertices);
